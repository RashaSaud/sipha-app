  ...
  try {
    ...
    if (userData) {
      ...
      const emailMessage = `
        ...
        <p>Please enter this code within [20] minutes to complete the verification process.</p>
...
const verification_otp = async (req, res) => {
  ...
  if (otp.length !== 6) {
    return res
      .status(400)
      .send({ message: "Invalid OTP length. Please provide a 6-digit code." });
  }
  ...
  try {
    ...
    if (!foundUser) {
      return res
        .status(404)
        .send({ message: "User not found. You need to sign up" });
    }
    ...
    if (otpMatches != otp) {
      return res
        .status(400)
        .send({ message: "Incorrect OTP. Please try again.." });
    }
